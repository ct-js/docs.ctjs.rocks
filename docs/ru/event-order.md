...
4. При начале игры или переходе игрока в новую комнату:
  2. Для каждой копии, добавленной в ассет комнаты, выполняется событие **Создание**;
  1. Вызывается событие **Старт комнаты**.
6. Затем начинается основной цикл игры:
    1. Вызывается событие **Начало кадра** для всех копий в комнате;
    2. Вызывается событие **Начало кадра** для текущей комнаты;
    3. Вызывается событие **Уничтожение** для всех копий, помеченных как «убить»;
    4. Копии пересортируются по глубине;
    5. Вызывается событие **Конец кадра** для всех копий;
    6. Вызывается событие **Конец кадра** для комнаты;
    7. Очищаются события ввода. Ожидание нового цикла игры.
7. Когда пользователь переходит в новую комнату, вызывается событие **Конец комнаты** для последней комнаты.
